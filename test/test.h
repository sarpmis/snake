...
void addLetter(char* board, struct Letter* letter, int offset){
	int i;
	int lettersize = letterSize(letter);
	if (offset < 6) {
		for (i = 0; i < lettersize; i++) {
			addToBoard(board,letter->points[i].x+offset,7,letter->points[i].z+1);
		}
	} else {
		for (i = 0; i < lettersize; i++) {
...
int letterSize(struct Letter* letter) {
	return sizeof(letter->points)/sizeof(struct Point);
}
